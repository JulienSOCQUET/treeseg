...
int main(int argc, char **argv)
{
	...
	if (top_dir[top_dir.length() - 1] != '/')
		top_dir = top_dir + "/";
	std::ifstream cfile;
	cfile.open(coordfileneame);
	float coordfile[4];
	int no_count = 0;
	...
	{
		for (float y = plot_ymin; y < plot_ymax; y += tile_length)
			tile_count++;
	}
	...
	while (tent = readdir(tdir))
		positions.push_back(tent->d_name);
	closedir(tdir);
	for (int k = 0; k < positions.size(); k++)
	{
		...
		if (positions[k][0] == 'S' && positions[k][4] == 'P')
		{
			...
			while (pent = readdir(pdir))
				position_contents.push_back(pent->d_name);
			closedir(pdir);
			std::string rxpname;
			...
			for (int l = 0; l < position_contents.size(); l++)
			{
				...
				//if(position_contents[l][14] == 'm' && position_contents[l][15] == 'o' && position_contents[l][16] == 'n' && position_contents[l].length() == 21)
				{
					...
					ss.str("");
					if (positions[k][7] == '0' && positions[k][8] == '0')
						ss << positions[k][9];
					else if (positions[k][7] == '0')
						ss << positions[k][8] << positions[k][9];
					else
						ss << positions[k][7] << positions[k][8] << positions[k][9];
					...
					std::cout << "Done, Total Points: " << totalcount / 1000000 << " Million" << std::endl;
				}
			}
		}
	}
	...
	for (int q = 0; q < tile_count; q++)
	{
		...
		if (basic == true)
			headerstream << "VERSION 0.7" << std::endl
						 << "FIELDS x y z" << std::endl
						 << "SIZE 4 4 4" << std::endl
						 << "TYPE F F F" << std::endl
						 << "COUNT 1 1 1" << std::endl
						 << "WIDTH " << tile_pointcount[q] << std::endl
						 << "HEIGHT 1" << std::endl
						 << "VIEWPOINT 0 0 0 1 0 0 0" << std::endl
						 << "POINTS " << tile_pointcount[q] << std::endl
						 << "DATA binary" << std::endl;
		if (basic == false)
			headerstream << "VERSION 0.7" << std::endl
						 << "FIELDS x y z range reflectance deviation return_number scan_number" << std::endl
						 << "SIZE 4 4 4 4 4 2 2 2" << std::endl
						 << "TYPE F F F F F U U U" << std::endl
						 << "COUNT 1 1 1 1 1 1 1 1" << std::endl
						 << "WIDTH " << tile_pointcount[q] << std::endl
						 << "HEIGHT 1" << std::endl
						 << "VIEWPOINT 0 0 0 1 0 0 0" << std::endl
						 << "POINTS " << tile_pointcount[q] << std::endl
